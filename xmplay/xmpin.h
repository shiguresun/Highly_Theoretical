...
typedef struct {
	...
	BOOL (WINAPI *VisOpen)(DWORD colors[3]); // vis opening or skin changed after (colors=background,text1,text2)
	void (WINAPI *VisClose)(); // vis closed
	void (WINAPI *VisSize)(HDC dc, SIZE *size); // get ideal vis dimensions
	BOOL (WINAPI *VisRender)(DWORD *buf, SIZE size, DWORD flags); // render vis (return false if unchanged)
	BOOL (WINAPI *VisRenderDC)(HDC dc, SIZE size, DWORD flags); // render vis (return false if unchanged)
	void (WINAPI *VisButton)(DWORD x, DWORD y); // mouse click in vis
	void (WINAPI *VisConfig)(HWND win); // present vis config options to user
